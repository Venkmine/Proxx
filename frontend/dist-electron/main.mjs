...
function writeLog(level, message) {
    ...
    ensureLogDir();
    if (fileLoggingDisabled)
        return; // Check again after ensureLogDir
...
function getErrorHtml(errorTitle, errorDetails) {
    return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
async function createWindow() {
    ...
    win.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
        ...
        win.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(getErrorHtml('Failed to Load', `Error ${errorCode}: ${errorDescription}\\n\\nURL: ${validatedURL}`))}`);
    });
    ...
    if (process.env.VITE_DEV_SERVER_URL) {
        ...
        try {
            ...
            // Show fallback error page instead of white screen
            win.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(getErrorHtml('Dev Server Unavailable', `Could not connect to Vite dev server at ${devUrl}\\n\\nMake sure the dev server is running:\\n  pnpm dev`))}`);
        }
        ...
        try {
            ...
            win.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(getErrorHtml('Failed to Load', `Could not load application from:\\n${prodPath}`))}`);
        }
    }
    ...
    win.webContents.on('render-process-gone', (_event, details) => {
        ...
        writeLog('ERROR', errorMsg);
        // Show fallback error page
        win.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(getErrorHtml('Renderer Crashed', `Reason: ${details.reason}\\nExit code: ${details.exitCode}`))}`);
    });
    win.on('unresponsive', () => {
