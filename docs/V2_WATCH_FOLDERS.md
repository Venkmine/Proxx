...
| **Unsupported source format** | → `failed/` with format rejection reason |
...
"ARRIRAW is not supported. Export ProRes or DNx from Resolve before proxy generation."
...
Watch folder processing validates source files against a formal **Source Capability Matrix**.
Unsupported formats are rejected **before execution** with actionable error messages.

### Supported Formats

| Container | Codecs |
|-----------|--------|
| `mp4` | h264, hevc, h265, av1 |
| `mov` | h264, hevc, h265, prores (all variants), dnxhd, dnxhr, mjpeg |
| `mkv` | h264, hevc, h265, vp9, av1 |
| `mxf` | dnxhd, dnxhr, mpeg2video, h264 (XAVC/XDCAM) |
| `webm` | vp9, av1 |
| `ts` | mpeg2video |
| `avi` | mjpeg |

### Rejected Formats (Hard Fail)

These formats will **always fail validation** with explicit guidance:

| Format | Example Error |
|--------|---------------|
| ARRIRAW | `"ARRIRAW is not supported. Export ProRes or DNxHR from DaVinci Resolve before proxy generation."` |
| REDCODE/R3D | `"RED RAW requires RED SDK. Export ProRes or DNxHR from Resolve or REDCINE-X."` |
| Blackmagic RAW | `"Blackmagic RAW requires manufacturer SDK. Export ProRes or DNxHR from DaVinci Resolve."` |
| Sony RAW/X-OCN | `"Sony RAW requires Sony SDK. Export ProRes or DNxHR from Sony RAW Viewer or Resolve."` |
| Canon Cinema RAW | `"Canon RAW requires Canon SDK. Export ProRes or DNxHR from Canon RAW Development or Resolve."` |
| ProRes RAW | `"ProRes RAW is sensor RAW, not standard video. Export standard ProRes from Final Cut Pro or Resolve."` |

### Why RAW is Not Supported

Camera RAW formats are sensor data, not video. They require proprietary SDKs for decode and produce non-deterministic output depending on debayering settings. The correct workflow is:

1. Import RAW into your NLE (DaVinci Resolve, Premiere, etc.)
2. Apply any color/exposure adjustments
3. Export to ProRes or DNxHR
4. Generate proxies from the exported intermediate

See [V2_JOB_SPEC.md](V2_JOB_SPEC.md#supported-source-formats) for the complete format matrix.
