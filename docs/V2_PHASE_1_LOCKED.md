...
**V2 Phase 2 is now active.** It adds bounded deterministic concurrency to the watch folder runner while preserving all Phase 1 guarantees.

### What Phase 2 Adds

- `--max-workers N` CLI flag for concurrent job processing
- Worker state tracking in result metadata
- Failure isolation (one job failing doesn't stop others)
- Deterministic ordering preserved (sorted filename dequeue)

See [V2_WATCH_FOLDERS.md](../V2_WATCH_FOLDERS.md) for Phase 2 concurrency documentation.
...
#### 1. **❌ NO Concurrency (SUPERSEDED BY PHASE 2)**
- ~~No parallel clip processing~~
- ~~No async execution~~
- ~~No threading or multiprocessing~~
- ~~Sequential execution only~~

**Update (Phase 2):** Bounded concurrency is now available as an opt-in feature:
- `--max-workers N` enables up to N concurrent jobs
- Each job still processes clips sequentially
- Determinism is preserved (sorted filename order)
- See `V2_WATCH_FOLDERS.md` for details

**Why:** Concurrency introduces complexity, race conditions, and non-determinism. Phase 1 prioritized correctness. Phase 2 adds controlled concurrency with explicit bounds.
